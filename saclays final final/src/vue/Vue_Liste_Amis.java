...
public class Vue_Liste_Amis extends JFrame {
	...
	private void afficher(Modele_Utilisateur utilisateur, boolean ami) {
		...
		IdButton boutonAmisCommuns = new IdButton("Amis en commun", utilisateur.getId());
		boutonAmisCommuns.setFont(Vue_Utilisateur.FONT);
		boutonAmisCommuns.setCursor(new Cursor(Cursor.HAND_CURSOR));	
		boutonAmisCommuns.setBackground(Vue_Accueil.MARRON);
		boutonAmisCommuns.setActionCommand(Controleur_Utilisateur.ACTION_VOIR_AMIS_COMMUNS);
		boutonAmisCommuns.addActionListener(controleurUtilisateur);
		...
		bouton.add(boutonAmisCommuns, gbc);
